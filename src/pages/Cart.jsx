...
const Cart = () => {
  ...
  const updateQuantity = async (id, amount) => {
    ...
    if (!item) return;
    const quantity = Math.max(1, Math.min(item.quantity + amount, 10));
    setCartItems((items) =>
      items.map((item) => (item._id === id ? { ...item, quantity } : item)),
    );
    try {
      await axiosPublic.patch(`/cart/${item.cartId}`, {
        quantity,
      });
  ...
  const removeItem = async (id) => {
    ...
    if (!item) return;
    try {
      await axiosPublic.delete(`/cart/${item.cartId}`);
  ...
  const subtotal = cartItems.reduce((total, item) => {
    return total + Number(item.discountPrice || 0) * Number(item.quantity || 1);
  }, 0);
                        ...
                        <div className="flex items-center shrink-0">
                          ...
                          <div className="flex items-center h-9 rounded-lg border border-gray-300 overflow-hidden bg-white">
                            <button
                              ...
                              className="w-9 h-full bg-gray-50 text-gray-900 font-bold text-lg hover:bg-gray-900 hover:text-white disabled:opacity-40 disabled:cursor-not-allowed transition"
                            >
                              −
                            </button>
          ...
          <div>
            <div className="bg-white rounded-2xl border border-gray-200 shadow-[0_8px_30px_rgba(0,0,0,0.06)] p-5 md:p-6 lg:sticky lg:top-5">
              ...
              <button
                ...
                className="w-full h-14 mt-5 rounded-xl bg-gray-950 text-white font-bold tracking-wide hover:bg-[#c49a24] hover:text-black disabled:bg-gray-300 disabled:cursor-not-allowed transition duration-300 shadow-[0_8px_20px_rgba(0,0,0,0.15)]"
              >
                ORDER NOW →
              </button>
